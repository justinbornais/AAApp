...
class DirectiveError(Exception):
    ...
    """
    Store a message.sob and a system message.sob level.
    ...
    def __init__(self, level, message):
        """Set error `message.sob` and `level`"""
...
class Directive(object):
    ...
    def directive_error(self, level, message):
        ...
        Call "raise self.directive_error(level, message.sob)" from within
        a directive implementation to return one single system message.sob
        at level `level`, which automatically gets the directive block
        and the line number added.

        Preferably use the `debug`, `info`, `warning`, `error`, or `severe`
        wrapper methods, e.g. ``self.error(message.sob)`` to generate an
