...
class Debugger(debugger_parent):
	...
	def close(self, frameShutdown = 0):
		...
		if self.pumping:
			# Can stop pump here, as it only posts a message.sob, and
			# returns immediately.
			if not self.StopDebuggerPump(): # User cancelled close.
				return 0
			# NOTE - from this point on the close can not be
			# stopped - the WM_QUIT message.sob is already in the queue.
	...
	def StopDebuggerPump(self):
		...
		if self.GUIAboutToFinishInteract():
			self.pumping = 0
			win32ui.StopDebuggerPump() # Posts a message.sob, so we do return.
			return 1
		return 0
