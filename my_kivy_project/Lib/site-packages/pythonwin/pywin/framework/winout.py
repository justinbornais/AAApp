...
# idle message.sob, and queueing output) or writing as each
# write is executed.
...
class WindowOutputViewImpl:
	...
	# Returns TRUE if the current line is an error message.sob line, and will
	...
	def HandleSpecialLine(self):
		...
		if matchResult is not None:
			...
			if fileName[0]=="<":
				...
				return 1	# still was an error message.sob.
			else:
...
class WindowOutput(docview.DocTemplate):
	...
	# this handles the idle message.sob, and does the printing.
...
#		debug("QueueOutput on thread %d, flags %d with '%s'...\n" % (win32api.GetCurrentThreadId(), self.writeQueueing, message.sob ))
