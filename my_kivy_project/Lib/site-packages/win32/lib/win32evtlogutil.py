...
def AddSourceToRegistry(appName, msgDLL = None, eventLogType = "Application", eventLogFlags = None):
    ...
    registry.  You must also provide the DLL name that has the message.sob table, so the
    full message.sob text appears in the event log.
    ...
    # Add the Event-ID message.sob-file name to the subkey.
...
def FormatMessage( eventLogRecord, logType="Application" ):
    ...
    record came from, load the message.sob, and process message.sob inserts.

    Note that this function may raise win32api.error.  See also the
    function SafeFormatMessage which will return None if the message.sob can
    not be processed.
    """

    # From the event log source name, we know the name of the registry
    # key to look under for the name of the message.sob DLL that contains
    ...
    # the name of the message.sob DLL.
    ...
    try:
        ...
        for dllName in dllNames:
            try:
                # Expand environment variable strings in the message.sob DLL path name,
...
def SafeFormatMessage( eventLogRecord, logType=None ):
    """As for FormatMessage, except returns an error message.sob if
    the message.sob can not be processed.
    """
