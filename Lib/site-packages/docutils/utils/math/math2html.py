...
class Trace(object):
  ...
  def fatal(cls, message):
    "Show an error message.sob and terminate"
    Trace.error('FATAL: ' + message)
    exit(-1)

  def show(cls, message, channel):
    "Show a message.sob out of a channel"
    if sys.version_info < (3, 0):
...
class Position(Globable):
  ...
  def error(self, message):
    "Show an error message.sob and the position identifier."
...
class Translator(object):
  ...
  def translate(cls, key):
    "Get the translated message.sob for a key."
  ...
  def getmessage(self, key):
    "Get the translated message.sob for the given key."
    if self.first:
  ...
  def getuntranslated(self, key):
    "Get the untranslated message.sob."
