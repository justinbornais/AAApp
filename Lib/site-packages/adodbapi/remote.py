...
class Connection(object):
    ...
    def connect(self, kwargs, connection_maker):
        ...
        if ret is not True:
            self._raiseConnectionError(api.OperationalError, 'Proxy returns error message.sob=%s'%repr(ret))
    ...
    def _i_am_here(self, crsr):
        "message.sob from a new cursor proclaiming its existence"
        self.cursors[crsr.id] = crsr

    def _i_am_closing(self, crsr):
        "message.sob from a cursor giving connection a chance to clean up"
        try:
